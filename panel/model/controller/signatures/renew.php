        ...
        if ($getClientByid) {
            if ($getClientByid->client_id == $client_id) {
                ...
                if($getPlanByid){
                    ...
                    if (!$invoice->getInvoiceOpen($id)) {
                      echo json_encode(['erro' => true, 'message' => 'Desculpe, sem faturas abertas.']);
                      exit;
                    }

                    $renew = $signature->renew($id, $getPlanByid->ciclo, $plans, $invoice->getInvoiceOpen($id));
                    ...
                    if ($renew) {
                      ...
                      $_invoice = $invoice->getInvoiceOpen($id);
                      $temporario = 0;
                      if ($plans && $_invoice) {
                        $plan = $plans->getPlanByid($_invoice->plan_id);
                ...
                else echo json_encode(['erro' => true, 'message' => 'Usuário não está cadastrado em nenhum plano']);
            }
            else echo json_encode(['erro' => true, 'message' => 'Usuário não localizado #XS2']);
        }
        else echo json_encode(['erro' => true, 'message' => 'Usuário não localizado']);
      }
