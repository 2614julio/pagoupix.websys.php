 ...
 @session_start();
  ...
  if(isset($_SESSION['CLIENT'], $_POST['type'])){
    ...
    try {
      ...
      if($mailVerify == 1){
          echo json_encode(['erro' => false, 'confirmed_last' => 1,  'message' => 'Vocêjá confirmou seu e-mail']);
          die;
      }
      ...
      if($type == "send"){
          ...
          if(!isset($_POST['code'])){
              echo json_encode(['erro' => true, 'message' => 'Informe o código que enviamos em seu e-mail']);
          }else{
              ...
              if(!isset($_SESSION['code_confirmation'])){
                   echo json_encode(['erro' => true, 'message' => 'Desculpe. Atualize a pagina e envie o código novamente.']);
                   die;
              }
              ...
              if(trim($_POST['code']) == $code_confirmation){
                  ...
                  if($mailVerifyOpt){
                      ...
                      if($options->editOption('mailVerify', 1)){
                          echo json_encode(['erro' => false, 'message' => 'Endereço de e-mail verificado com sucesso!']);
                      }else{
                          echo json_encode(['erro' => true, 'message' => 'Informe o código que enviamos em seu e-mail']);
                      }
                  }else{
